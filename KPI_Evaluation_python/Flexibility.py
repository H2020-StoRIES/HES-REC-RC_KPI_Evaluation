...
import random
import numpy as np
from metriccalculator import Metric


class Flexibility(Metric):

    def __init__(self, data, parameters):
        super().__init__( data, parameters)
        # Price_market: negative to positive
        self.sorted_indices = np.argsort(Price_market)
        sorted_Price_market = Price_market[self.sorted_indices]
        sorted_P_el_in = P_el_in[self.sorted_indices]
        sorted_P_el_out = P_el_out[self.sorted_indices]
        sorted_P_delta = P_delta[self.sorted_indices]

        # Price_tarrif: positive
        self.sorted_indices1 = np.argsort(Price_tarrif)
        sorted_Price_tarrif = Price_tarrif [self.sorted_indices1]
        sorted_P_cons = P_cons[self.sorted_indices1]
        sorted_P_ESS = P_ESS[self.sorted_indices1]
    ...
    def calculate(self):
        metrics={}
        metrics['FF_Pdelta']=self.FF_Pdelta()
        return self.metrics